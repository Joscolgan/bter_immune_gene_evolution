...
wilcox.test(immune_genes_df$length_corrected_theta,
            non_immune_genes_df$length_corrected_theta)

wilcox.test(subset(x = new_combined_df,
                   status == "immune")$length_corrected_theta,
            subset(x = new_combined_df,
                   status == "non-immune")$length_corrected_theta)
```
...
contingency_table_chi <- chisq.test(contingency_table)
```
...
## Generate the number of intronic SNPs per immmune and non-immune genes:
## Construct a contingency table:
intron <- rbind(c(8485, 620407),
                c(12835 - 8485 - 3028,
                  685034 - 620407 - 12835 - 3028))
## Perform a chi-squared test:
...
## Perform the same for SNPs in up/downstream regions:
## Construct a contingency table:
...
## Perform the same for SNPs in untranslated regions (UTRs):
## Construct a contingency table:
...
To determine whether canonical immune genes are identified as under selection
