...
## Define function to check if input files are populated:
...
## Define function to check if software tools exist (i.e., path is correct):
...
## Define function to check if the header information of each input files match:
def check_identical_fastq_headers(file1, file2):
    ...
    subprocess.call("grep '@A00' %s | cut -d ' ' -f 1 - > %s" % (file1, head1[1]), shell=True)
    subprocess.call("grep '@A00' %s | cut -d ' ' -f 1 - > %s" % (file2, head2[1]), shell=True)
