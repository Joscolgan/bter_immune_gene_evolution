...
with open('CNV_duplications.txt') as duplications:
...
DUPLICATED_DATA         = "filter_duplicated_beds/{duplications}.bed"

# Output intersected BAM files here.
INTERSECTED_DATA        = "filtered_dup_temp/01_intersect_bam/{duplications}.{samples}.bam"

# Output depth counts here.
DEPTH_DATA              = "filtered_dup_temp/02_depth_bam/{duplications}.{samples}.depth.txt"

# Output mean counts per sample here.
CALCULATED_MEDIAN_DATA  = "filtered_dup_temp/03_median_counts/{duplications}.{samples}.median_depth.txt"

# Output combined mean data here.
COMBINED_MEDIAN_DATA    = "filtered_dup_temp/04_combined_counts/{duplications}.combined.median_depth.txt"

# Output combined CNV mean data here.
COMBINED_CNV_DATA       = "filtered_dup_temp/05_combined_CNV_means/combined.median_depth.all_CNVs.txt"

# Output final data file here.
FINAL_DATA              = "filtered_dup_temp/06_final/final_combined.median_depth.all_dups.txt"
...
rule paste_means:
    input:  expand("filtered_dup_temp/03_median_counts/{{duplications}}.{samples}.median_depth.txt", samples = SAMPLES)
...
rule combine_CNVs:
    input:  expand("filtered_dup_temp/04_combined_counts/{duplications}.combined.median_depth.txt", duplications = DUPLICATIONS)
